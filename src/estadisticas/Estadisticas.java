...
public class Estadisticas {
    ...
    /**
     * This method calculates the standardDeviation of a specified column given
     * a matrix.
     *
     * @param obj real number matrix
     ...
     */
    public double best(double[] obj ) {//agregar double []obj2 double[] obj2
        if (obj == null ) {
            System.err.println("Error: empty vector.");
            return Double.NaN;
        }
        obj = bubble(obj); //mandar obj, obj2
        return obj[0];
    ...
    public double[] bestRow(double[][] obj, int column) {
        ...
        for (double[] obj1 : obj) {
            if (obj1[column] == best && obj1[column+1]==0) {
                return obj1;
            }
        }
        return null;
...
//    && obj1[column+1]==0
    ...
    /**
     * This method calculates the worst Row of a specified column given a
     * matrix.
     *
     * @param obj real number matrix
     ...
     */
//    && obj[j+1]==0
    private double[] bubble(double[] obj) { //agregar, double obj2
        double assistant;
        for (int i = 0; i < obj.length - 1; i++) {
            for (int j = (i + 1); j < obj.length; j++) {
                if (obj[j] < obj[i] ) {
                    //if((obj[j]< obj[i]) && (obj2[j]< obj2[i]))
                    assistant = obj[i];
                    obj[i] = obj[j];
                    obj[j] = assistant;
                }
    ...
    /**
     * This method calculates the feasible Rate of a specified column given a
     * matrix.
     *
     * @param obj real number matrix
    ...
    /**
     * This method calculates the success Rate of a specified column given a
     * matrix.
     *
     * @param obj real number matrix
    ...
    /**
     * This method calculates the success Performance of a specified column
     * given a matrix.
     *
     * @param obj real number matrix
     * @param bestKnownValue best known value of CNOP
    ...
    /**
     * This method calculates the distance between the vector values and the
     * mean.
     *
     * @param obj real number vector
    ...
    /**
     * Method to calculate the chosen statistics and to be able to reuse storage
     * vectors.
     *
     * @param obj real number vector
     * @param option :mean : 0, standardDeviation: 1, median: 2, best: 3, worst:
     * 4
     * @return real value
     */
    ...
    /**
     * mean : 0 standardDeviation: 1 median: 2 best: 3 worst: 4
     *
     * @param obj
     * @param column
     * @param option
    ...
    private double evaluate(double[] vectorDouble, final int option) {
        switch (option) {
            case 0:
                return this.mean(vectorDouble);
            case 1:
                return this.standardDeviation(vectorDouble);
            case 2:
                return this.median(vectorDouble);
            case 3:
                return this.best(vectorDouble);
            case 4:
                return this.worst(vectorDouble);
            default:
                System.err.println("Error: option invalid.");
        }
        return Double.NaN;
    } // End evaluate     
    ...
    public int CorridasFact(double[][] obj) {
        this.contCF=0;
        for (int i = 0; i < obj.length; i++) {
            if (obj[i][obj[0].length - 1] == 0) {
                contCF++;
            }
        }
        return contCF;
      ...
      public double porCorrFact(double[][] obj) {

        return (contCF*100)/ obj.length;
    }
    

    public double porcCorrExt(double[][] obj, double bestKnownValue) {
        double xCorrEx = 0;
        int contCE=0;
        for (int i = 0; i < obj.length; i++) {
            if ((obj[i][obj[0].length - 1] == 0) && ((obj[i][obj[0].length-2]-(bestKnownValue))<=0.0001)){
                contCE++;
            }
        }

        xCorrEx=(contCE*100)/obj.length;

        return xCorrEx;

    }
    
     public double successPerform(double[][] obj, double bestKnownValue, double [][] individuos) {
        double SuccessP = 0;
